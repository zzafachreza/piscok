...
export default function AASetor({ navigation, route }) {
    ...
    const sendServer = () => {
        ...
        if (parseFloat(kirim.nominal) > 0) {
            setLoading(true);
            axios.post(apiURL + 'setor_saldo', kirim).then(res => {
                console.log(res.data);
                if (res.data == 200) {
                    Alert.alert(MYAPP, 'Setor Saldo ' + kirim.showNominal + ' berhasil di simpan !');
                    navigation.goBack();
                }
            });
        } else {
